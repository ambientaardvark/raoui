...
- Reads are deny-default (scoped to R + cwd) with a trailing deny over known `$HOME` credential stores; writes/network/exec still use the allow-default-then-deny structure. The secret-deny list is itself a blocklist (`~/.ssh`, `~/.aws`, `~/.gnupg`, `~/.config`, `~/Library/Keychains`, `.Renviron`, shell histories, `~/.docker`, `~/.kube`) — high-value locations, not exhaustive; a determined long-tail secret outside both cwd and the list could still be read. A user-configurable extra-read-paths knob (for data outside cwd) is a natural follow-on.
