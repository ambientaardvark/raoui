...
module type LEXER = sig
  ...
  (** Lex a single line, given starting mode. Returns tokens and the mode at end
      of line. *)

  val lex_as_default : string -> token
  (** lex entire line as the default token. Useful when you do not want syntax
      highlighting *)
end
...
module Make (L : LEXER) : sig
  ...
  (** Update cache when lines change.
      ...
      [update ~start_line ~end_line ~lines cache] efficiently recomputes the
      cache when lines [start_line] through [end_line] have changed.
...
val cursor_byte_offset : line:Unicode_string.t -> cursor_pos:int -> int
(** Calculate the byte offset of a cursor position within a line. Needed for
    token-based analysis since tokens use byte positions. *)

(** Find tokens around a specific byte position (for context-aware features) *)
val tokens_with_positions :
  'token list -> token_to_lexeme:('token -> string) -> ('token * int * int) list
