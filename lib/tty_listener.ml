...
let read_byte_timeout clock stdin timeout_sec =
  match Eio.Time.with_timeout clock timeout_sec (fun () -> Ok (read_byte stdin)) with
  | Ok result -> result
  | Error `Timeout -> None
...
let read_bracketed_paste stdin =
  ...
  let rec loop () =
    ...
    | Some '\x1b' ->
      ...
      (match read_byte stdin, read_byte stdin, read_byte stdin, read_byte stdin, read_byte stdin with
