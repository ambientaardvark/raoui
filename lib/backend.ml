...
let kernel_path = "/Users/alanlee/Documents/Programs/raoui/vendor/ark-0.1.223-darwin-universal/ark"
...
let create () =
  ...
  Zmq.Socket.connect shell (Printf.sprintf "tcp://127.0.0.1:%d"
    Yojson.Basic.Util.(conn_info |> member "shell_port" |> to_int));
  Zmq.Socket.connect iopub (Printf.sprintf "tcp://127.0.0.1:%d"
...
let recv_message socket =
  ...
  let rec read_message parts =
    match parts with
    | delim::_signature::header::_parent_h::_metadata::content::_buffers
      when String.equal delim "<IDS|MSG>" -> (header, content)
...
let cancel t =
  Caml_unix.kill (Core.Pid.to_int t.ark_pid) Stdlib.Sys.sigint
