...
let view model =
  ...
  Buffer.add_string buf
    (if model.awaiting_response then hide_cursor else show_cursor );
  ...
  scroll_terminal buf model.scroll_amount;
  ...
  List.iteri (fun i line ->
    if i >= skip_rows && i < skip_rows + model.term_height then begin
      ...
      if i < skip_rows + model.term_height - 1 && i < total_rows - 1 then Buffer.add_char buf '\n'
    end
  ) wrapped;
