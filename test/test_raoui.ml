...
let test_resize_crash () =
  ...
  try
    let res = Update.update (Tty_listener.Char 'b') ~term_width:new_width model in
    match res with
...
let test_submit_blocked_while_awaiting () =
  let width = 10 in
  let model = { (initial_model width) with awaiting_response = true; lines = ["test"] } in
...
let test_backspace_while_awaiting () =
  let width = 10 in
  let model = { (initial_model width) with awaiting_response = true; lines = ["ab"]; cursor_col = 2 } in
...
let test_backspace_at_start () =
  ...
  match Update.update Tty_listener.Backspace ~term_width:width model with
  | Continue new_model ->
    Alcotest.(check string) "empty line unchanged" "" (List.hd new_model.lines)
  | _ -> Alcotest.fail "Expected Continue"
...
let test_backspace_merges_lines () =
  ...
  | Continue new_model ->
    ...
    Alcotest.(check string) "content merged" "helloworld" (List.hd new_model.lines)
  | _ -> Alcotest.fail "Expected Continue"
