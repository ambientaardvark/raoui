...
let test_glue_simple () =
  let result = Syntax.parse_glue_string "\"hello\"" in
  check_spans "simple string"
    [ (`String, "\""); (`String, "hello"); (`String, "\"") ]
    result

let test_glue_interpolation () =
  let result = Syntax.parse_glue_string "\"hello {name}\"" in
...
let test_glue_escaped_open () =
  let result = Syntax.parse_glue_string "\"{{literal\"" in
  check_spans "escaped {{"
...
let test_glue_expr_then_escaped () =
  ...
  let result = Syntax.parse_glue_string "\"hello, {a}}}\"" in
...
let test_glue_nested_braces () =
  ...
  let result = Syntax.parse_glue_string "\"{if (x) { 1 } else { 2 }}\"" in
...
let test_glue_multiple_interpolations () =
  let result = Syntax.parse_glue_string "\"{a} and {b}\"" in
...
let test_glue_complex_expr () =
  let result = Syntax.parse_glue_string "\"{x + 1}\"" in
...
let test_function_detection () =
  ...
  let spans = Syntax.tokens_to_spans tokens in
...
let test_highlight_simple () =
  let spans, mode = Syntax.highlight_line Lexer.Normal "x <- 1" in
...
let test_highlight_with_glue () =
  let spans, _ = Syntax.highlight_line Lexer.Normal "glue(\"{x}\")" in
  let text = String.concat "" (List.map snd spans) in
